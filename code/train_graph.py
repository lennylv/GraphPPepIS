...
def train():
    ...
    for batch in range(0, len(fea)):
        ...
        adj_mat = get_adjacency_matrix(dic[batch], args.cutoff, pro_length)
        ...
        optimizer.zero_grad()
        output_pro, output_pep = model(node_fea_pro, adj_mat_pro), model(node_fea_pep, adj_mat_pep)
        loss_tra_pro, loss_tra_pep = loss_fcn(output_pro, label_pro), loss_fcn(output_pep, label_pep)
...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), os.path.join("../model", 'modelgraph%.1f_%.1f_%d_%d_%d.dat'%(args.pw, args.lw, args.epochs, args.units, args.layers)))
