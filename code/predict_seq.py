...
from train_seq import layers, units, dropout, lamda, alpha, variant, cutoff, threshold
...
def test(pair_num_pro, pair_num_pep):
    ...
    with torch.no_grad():
        for batch in range(0, len(fea)):
            ...
            adj_mat_pro = get_adjacency_matrix(dic[batch], cutoff, pro_length)
            ...
            output_pro, output_pep = torch.sigmoid(output[:pro_length]), torch.sigmoid(output[pro_length:])
            ...
            for pro_index in range(len(label_pro)):
                pro_pred, pro_label = (output_pro[pro_index]>0.5).astype(int), label_pro[pro_index].astype(int)
            ...
            for pep_index in range(len(label_pep)):
                pep_pred, pep_label = (output_pep[pep_index]>0.5).astype(int), label_pep[pep_index].astype(int)
                TP += (pep_pred&pep_label)
                TN += ((pep_pred==0)&(pep_label==0))
                FP += (pep_pred&(pep_label==0))
                FN += ((pep_pred==0)&pep_label)
            if (TP+FP)==0 or (TN+FN)==0 or (TN+FP)==0 or (TN+FN)==0:
                mcc, au_roc = 0, 0
                pair_num_pep -= 1
            else:
                mcc = (TP*TN-FP*FN)/math.sqrt((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN))
...
if __name__ == '__main__':
    ...
    model_file = '../model/modelseq0.8_1.0_300_512_8.dat'
